...
func (cg *ConfigGenerator) generatePodMonitorConfigForAgentDaemonSet(
	m *monitoringv1.PodMonitor,
	ep monitoringv1.PodMetricsEndpoint,
	i int, apiserverConfig *monitoringv1.APIServerConfig,
	store *assets.StoreBuilder,
	...
	// DaemonSet: Need to add before this => But where?
...
func (cg *ConfigGenerator) appendPodMonitorConfigsForAgentDaemonSet(
	slices []yaml.MapSlice,
	podMonitors map[string]*monitoringv1.PodMonitor,
	apiserverConfig *monitoringv1.APIServerConfig,
	...
	for _, identifier := range pMonIdentifiers {
		for i, ep := range podMonitors[identifier].Spec.PodMetricsEndpoints {
			slices = append(slices,
				cg.WithKeyVals("pod_monitor", identifier).generatePodMonitorConfigForAgentDaemonSet(
					podMonitors[identifier], ep, i,
					apiserverConfig,
					store,
				),
...
func (cg *ConfigGenerator) appendAdditionalScrapeConfigsForAgentDaemonSet(scrapeConfigs []yaml.MapSlice, additionalScrapeConfigs []byte) ([]yaml.MapSlice, error) {
	addlScrapeConfigs, err := cg.generateAdditionalScrapeConfigsForAgentDaemonSet(additionalScrapeConfigs)
	if err != nil {
		return nil, err
	}
...
func (cg *ConfigGenerator) GenerateAgentDaemonSetConfiguration(
	...
	scrapeConfigs = cg.appendPodMonitorConfigsForAgentDaemonSet(scrapeConfigs, pMons, apiserverConfig, store)

	scrapeConfigs, err := cg.appendAdditionalScrapeConfigsForAgentDaemonSet(scrapeConfigs, additionalScrapeConfigs)
	if err != nil {
		return nil, fmt.Errorf("generate additional scrape configs: %w", err)
	}
	cfg = append(cfg, yaml.MapItem{
